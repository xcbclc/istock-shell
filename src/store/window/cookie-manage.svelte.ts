...
export const getCookieManage = (ctx: CmdWindowContext) => {
  ...
  cookieManage.getList = async (query) => {
    const { payload } = await ctx.message.send<TCookieModel[]>('setting', 'cookie.list', query);
  ...
  cookieManage.create = async (data) => {
    ...
    await ctx.message.send('setting', 'cookie.create', newData);
  };
  cookieManage.edit = async (data) => {
    const { isEdit, ...newData } = data;
    await ctx.message.send('setting', 'cookie.update', newData);
  };
  cookieManage.delete = async (id) => {
    const { payload } = await ctx.message.send<boolean>('setting', 'cookie.delete', id);
