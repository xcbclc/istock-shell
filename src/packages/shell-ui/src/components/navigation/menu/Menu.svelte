...
<script lang="ts" module>
  ...
  /** 全局设置选择激活状态Key，用于指定激活菜单项 */
  export const MENU_ROOT_SELECTED_KEY = Symbol('menu.root.selected.key');
</script>
...
<script lang="ts">
  ...
  let {
    ...
    items = [], // 菜单项数据列表，默认空数组
    selectedKey = $bindable(), // 被选择激活的菜单项key
  ...
  if (isRoot) {
    ...
    setContext(MENU_ROOT_SELECTED_KEY, (key?: string) => {
      if (!key) return selectedKey;
      if (selectedKey === key) return selectedKey;
      selectedKey = key;
      return selectedKey;
    });
  }
  ...
  // 获取全局被选择激活的菜单项key
  let rootSetSelectedKey = getContext<(k?: string) => string>(MENU_ROOT_SELECTED_KEY);
  ...
  {:else}
    ...
    {#each items as item}
      <!--
        ...
        确保激活状态的一致性和可控性
      -->
      {@const active = rootSetSelectedKey() ? rootSetSelectedKey() === item.key : item.active}
      <!--
        渲染菜单项组件
