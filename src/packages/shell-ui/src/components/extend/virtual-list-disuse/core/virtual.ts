...
export class Virtual {
   ...
   */
  scrollViewByIndex(index: number, align: 'start' | 'center' | 'end' | 'nearest' = 'start') {
    // 边界检查
    if (index < 0 || index >= this.#dataSources.length) return;

    // 获取目标元素的偏移量（包含header偏移）
    const targetOffset = this.#virtualCore.getOffset(index);
    const clientSize = this.getClientSize();
    const currentOffset = this.getOffset();

    // 获取目标元素的实际尺寸
    let elementSize = this.#virtualCore.getEstimateSize(); // 默认使用预估尺寸

    // 尝试获取目标元素的实际尺寸
    if (index < this.#dataSources.length) {
      const item = this.#dataSources[index];
      const dataKey = this.#dataKey;
      const elementId = typeof dataKey === 'string' ? item[dataKey] : dataKey(item, index);
      const actualSize = this.#virtualCore.getSizeById(elementId);
    ...
    // 确保偏移量在有效范围内
    const maxOffset = this.getScrollSize() - clientSize;
    finalOffset = Math.max(0, Math.min(finalOffset, maxOffset));

    // 执行滚动
    this.scrollToOffset(finalOffset);
  }
