<script lang="ts" module>
  ...
  // 定义工具提示位置类型（从配置中提取）
  export type TooltipPosition = keyof (typeof tooltipVariantConfig)['variants']['position'];

  // 组件属性接口（继承div元素属性）
  export interface TooltipProps extends HTMLAttributes<HTMLDivElement> {
    color?: TooltipColor; // 颜色主题
    position?: TooltipPosition; // 显示位置
    open?: boolean; // 是否显示
    dataTip?: string; // 提示文本
...
<div class={[tuc(tooltipVariants({ color, position, open })), className]} data-tip={dataTip ?? ''} {...otherProps}>
  {#if tooltipRender}
    <!-- 自定义提示内容 -->
