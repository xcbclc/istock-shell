[**@istock-shell/command-parser**](README.md)

---

# @istock-shell/command-parser

## Enumerations

- [AstTreeType](enumerations/AstTreeType.md)
- [TokenType](enumerations/TokenType.md)

## Classes

- [Ast](classes/Ast.md)
- [CmdParser](classes/CmdParser.md)
- [Tokenizer](classes/Tokenizer.md)

## Interfaces

- [Argument](interfaces/Argument.md)
- [Command](interfaces/Command.md)
- [Option](interfaces/Option.md)

## Type Aliases

- [AstTree](type-aliases/AstTree.md)
- [AstTreeCommand](type-aliases/AstTreeCommand.md)
- [AstTreeItem](type-aliases/AstTreeItem.md)
- [AstTreeKeyCommand](type-aliases/AstTreeKeyCommand.md)
- [AstTreeMention](type-aliases/AstTreeMention.md)
- [AstTreeOption](type-aliases/AstTreeOption.md)
- [AstTreeParameter](type-aliases/AstTreeParameter.md)
- [AstTreeParentheses](type-aliases/AstTreeParentheses.md)
- [AstTreePipe](type-aliases/AstTreePipe.md)
- [AstTreeRoot](type-aliases/AstTreeRoot.md)
- [CommandItemResult](type-aliases/CommandItemResult.md)
- [CommandParenthesesResult](type-aliases/CommandParenthesesResult.md)
- [CommandParserResult](type-aliases/CommandParserResult.md)
- [CommandPipeResult](type-aliases/CommandPipeResult.md)
- [CommandResult](type-aliases/CommandResult.md)
- [KeyCommandResult](type-aliases/KeyCommandResult.md)
- [Token](type-aliases/Token.md)

## Variables

- [keyCommand](variables/keyCommand.md)
