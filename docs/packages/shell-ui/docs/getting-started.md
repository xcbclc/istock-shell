...
<script>
  import { ShInput, ShTextarea, ShSelect, ShCheckbox, ShButton, ShAlert, ShFieldSet } from '@istock-shell/ui';
...
<div class="max-w-md mx-auto p-6 space-y-6">
  ...
  <ShFieldSet legend="基本信息">
    <div class="space-y-4">
      ...
      <ShInput bind:value={formData.email} type="email" placeholder="请输入邮箱地址" label="邮箱" required />
    </div>
  </ShFieldSet>
  ...
  <ShFieldSet legend="密码设置">
    <div class="space-y-4">
      <ShInput bind:value={formData.password} type="password" placeholder="请输入密码" label="密码" required />
  ...
  <ShFieldSet legend="个人信息">
    <div class="space-y-4">
      <ShSelect bind:value={formData.role} options={roleOptions} placeholder="请选择角色" label="角色" required />

      <ShTextarea bind:value={formData.bio} placeholder="简单介绍一下自己..." label="个人简介" rows={3} />
    </div>
  </ShFieldSet>
  ...
  <!-- 提交按钮 -->
  <ShButton color="primary" size="lg" block loading={isSubmitting} disabled={isSubmitting} onclick={handleSubmit}>
...
<!-- DataDisplay.svelte -->
<script>
  import { ShTable, ShStat, ShButton, ShInput, ShSelect, ShLoading, ShEmpty } from '@istock-shell/ui';
...
<div class="p-6 space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
    ...
    <ShStat title="本月新增" value={stats.newThisMonth} description="本月新注册用户" color="warning" />
  </div>
...
<div class="p-6 space-y-6">
  ...
  <div class="mt-8 p-4 bg-base-200 rounded-lg">
    ...
    <p class="text-sm text-base-content/70 mt-2">主题会影响所有组件的颜色和样式，你可以通过切换不同主题来查看效果。</p>
  </div>
</div>
```
...
<div class="p-4 lg:p-6 space-y-6">
  ...
  <!-- 提示信息 -->
  <ShAlert color="info">💡 提示：这个仪表盘演示了响应式布局的使用。在不同屏幕尺寸下，组件会自动调整布局。</ShAlert>
</div>
```
