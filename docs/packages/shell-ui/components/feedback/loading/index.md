...
通过 `shape` 和 `size` 属性实现加载指示器的样式定制。Loading 组件支持六种动画形状（spinner/dots/ring/ball/bars/infinity）和五种标准尺寸（xs/sm/md/lg/xl），提供丰富的视觉效果选择，默认中等尺寸，适用于不同界面密度和用户体验需求的加载状态展示场景。
