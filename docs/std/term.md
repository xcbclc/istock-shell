...
## 2. 架构与框架 (Architecture & Framework)
...
在任何应用上下文中均可执行的命令，通常用于系统管理、环境切换或通用工具（如 `cls`, `help`, `exit`）。

### Application Command (应用命令)

仅在特定应用被激活（Active）或指定应用前缀时才能执行的命令。
...
源代码（此处指命令行输入）的树状表现形式。Parser 将线性 Token 流转换为 AST，以便准确理解命令的结构和嵌套关系。

### Validator (验证器)

命令解析的第三阶段，根据命令定义（Schema）检查解析后的对象是否合法（例如参数类型是否匹配、必选参数是否缺失）。

## 4. 协议与通信 (Protocol & Communication)
...
用于在 CMDP 协议中唯一标识通信端点。格式通常为 `processId:threadId` 或类似的层级结构。

## 5. 数据与存储 (Data & Storage)
